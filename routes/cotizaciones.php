...
Route::middleware(['roles'=>'allow_to_roles:admin|super_admin'])->group(function () {
		...
		return DataTables::eloquent($items)
				...
				->addColumn('acciones', function($item){ 
					...
					if (!is_null($item->cotizacion_uuid)){
						$btn_autorizar = route('cotizaciones.autorizar',$item_id);
						$btn_autorizar = "
	...
	Route::get('cotizaciones.autorizar/{uuid}','CotizacionesController@autorizar')
		->name('cotizaciones.autorizar');

	Route::get('autorizaciones','CotizacionesController@por_autorizar')
		->name('autorizaciones');
	...
	Route::match(['get', 'post'],'por_autorizar', function() {
		...
		$items = Autorizacion::query()
					...
					->whereNot('estatus_id',$estatus->id)
